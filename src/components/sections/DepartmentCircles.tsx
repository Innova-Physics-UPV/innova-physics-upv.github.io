    ...
    return (
      ...
      >
        <div className="relative flex justify-center">
          {/* Logo principal del departamento */}
          <button
          ...
          >
            <img
              ...
              alt={department.name}
              loading="eager" // importante para la carga inmediata del logo principal
              className="w-[clamp(24px,10vw,120px)] h-[clamp(24px,10vw,120px)] object-contain filter invert dark:invert-0"
            />
          </button>

        {/* Subdepartamentos: renderizados solo si el departamento está abierto */}
        {isOpen && (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            {department.subDepartments.map((subDept, idx) => {
              ...
              const radius = 12;
              const x = isOpen ? `calc(${Math.cos(angle) * radius}vw)` : '0';
              const y = isOpen ? `calc(${Math.sin(angle) * radius}vw)` : '0';
              ...
              return (
                <div
                  ...
                  style={{
                    transform: `translate(${x}, ${y}) scale(${isOpen ? 1 : 0})`,
                    opacity: isOpen ? 1 : 0,
                ...
                >
                  <button
                    ...
                    aria-label={`Ir a la sección ${subDept.name}`}
                  >
                    <img
                      src={subDept.image}
                      alt={subDept.name}
